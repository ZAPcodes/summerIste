...
import { ArrowLeft, Palette, BookOpen, PlayCircle, ExternalLink, CheckCircle, Clock, Target, ChevronDown, ChevronRight, FileText, Trophy, AlertCircle } from "lucide-react";
...
import { designCurriculum, Resource as CurriculumResource, CurriculumTaskData, CurriculumWeekData } from "@/data/designCurriculum";
...
const Design = () => {
  ...
  const { progress, loading, error, toggleTask, updateQuizProgress } = useProgress("design");
  ...
  const { status: quizStatus, loading: scheduleLoading } = useQuizSchedule("design", currentQuizWeek || 1);

  const staticCurriculum: CurriculumWeekData[] = designCurriculum;
  ...
  const getResourceIcon = (type: "video" | "article" | "documentation") => {
    switch (type) {
      case "video":
        return <PlayCircle className="w-4 h-4 text-pink-400" />;
      case "article":
  ...
  const handleQuizComplete = async (score: number, totalQuestions: number, answers: { [key: string]: number }, timeUsed: number) => {
    const quizData = designQuizzes.find((q) => q.weekId === currentQuizWeek);
    if (!quizData) return;
    ...
    try {
      await updateQuizProgress("design", currentQuizWeek!, passed, score);
    } catch (err) {
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => navigate(`/leaderboard/design/${week.id}`)}
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-pink-900 to-black text-white p-6">
  ...
  if (quizState === "in_progress" && currentQuizWeek) {
    ...
    if (quizData) {
      ...
      return <QuizInterface quiz={enhancedQuizData} onComplete={handleQuizComplete} />;
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-pink-900 to-black text-white p-6">
      <div className="container mx-auto max-w-4xl">
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold mb-2">ISTE UI/UX Design Course</h1>
          <p className="text-gray-300">Craft intuitive and beautiful user interfaces with ISTE</p>
        </div>

        <Card className="bg-black/40 backdrop-blur-sm border-pink-800/50 mb-8">
          <CardHeader>
        ...
        <div className="space-y-6">
          {weeks.map((week, weekIndex) => (
            <Card 
              ...
              className={`${
                week.isUnlocked 
                  ? "bg-black/40 backdrop-blur-sm border-pink-800/50" 
                  ...
                  <div className={`flex items-center justify-between ${week.isUnlocked ? 'cursor-pointer' : 'cursor-not-allowed'}`}>
                    <CollapsibleTrigger asChild>
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${week.quizCompleted ? "bg-green-500" : week.isUnlocked ? "bg-pink-500" : "bg-gray-600"}`}>
                          {week.quizCompleted ? (
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-lg font-semibold text-pink-400">{Math.round(week.progress)}%</div>
                              ...
                              {week.resources.map((resource, resourceIndex) => (
                                <div key={resourceIndex} className="flex items-center gap-3 p-3 bg-gray-800/20 rounded border border-gray-700/30">
                                  ...
                                  <Button 
                                    size="sm" 
                                    variant="ghost" 
                                    className="h-8 px-3 text-pink-400 hover:text-pink-300"
